...
func PoolStatusViewHandler(db *gorm.DB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			handlers.ErrorHandler(w, r, "Could not verify signature and address for request", err, http.StatusForbidden)
			return
		}

		profile, err := getProfile(signedMessage, db)
		if err != nil {
			handlers.ErrorHandler(w, r, "Could not retrieve profile for wallet: "+signedMessage.Address, err, http.StatusBadRequest)
			return
		}
		...
		handlers.ResponseHandler(w, r, "null", true, nil, response, nil)
	}
}
...
func PoolNewApplicationHandler(db *gorm.DB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			handlers.ErrorHandler(w, r, "Could not verify request payload", err, http.StatusBadRequest)
			return
		}

		profile, err := controller.NodeApplyToPool(db, requestPayload)
		if err != nil {
			handlers.ErrorHandler(w, r, "Could not apply to pool", err, http.StatusBadRequest)
			return
		}
